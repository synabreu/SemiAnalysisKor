...
|-------------------------------------------------------------------------------|
| <span style="color:green">참고: 비인과적 어텐션 레이어(non-causal attention layers)는 모델이 입력 데이터의 모든 부분을 참조할 수 있도록 허용하는 어텐션 메커니즘이다. 따라서, 이전 토큰까지만 참조할 수 있어 시퀀스 데이터의 시간적 순서를 보존할 수 있다. 주로 언어 모델의 텍스트 생성 작업에 사용되는 데, 긴 문서나 복잡한 데이터 구조에서 모든 정보를 참조해야 할 때 유용하다. 또한, FlexAttention 같은 최적화된 API를 통해 비인과적 어텐션에서도 효율적인 연산을 가능하게 한다. 적용 사례로, 특정 문서 내에서 중요한 부분에만 집중하는 문서 마스킹, 길이가 긴 시퀀스에서 국소적인 정보만 참조하는 슬라이딩 윈도우 어텐션, 그리고 위치 정보(포지셔널 인코딩)를 활용하여 학습 성능을 개선하는 알리바이 등에 사용한다.</span> |
