...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-02.webp)
[Source: SemiAnalysis]
...
![Source: NVIDIA](./20241222-mi300x/mi300x-05.webp)
[Source: NVIDIA]
...
![Source: SemiAnalysis]((./20241222-mi300x/mi300x-04.webp)
[Source: NVIDIA]
...
![Source: Github](./20241222-mi300x/mi300x-09.webp)
[Source: SemiAnalysis]
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-10.webp)
[Source: SemiAnalysis]
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-12.webp)
...
![Source: SemiAnalysis, AMD](./20241222-mi300x/mi300x-13.webp)
[Source: SeminAnalysis, AMD]

![Source: Nvidia](./20241222-mi300x/mi300x-14.webp)
[Source: NVIDIA]
...
![Source: Nvidia](./20241222-mi300x/mi300x-15.webp)
[Source: NVIDIA]
...
![Source: SemiAnalysis, AMD](./20241222-mi300x/mi300x-16.webp)
...
![Source: SeminAnalysis](./20241222-mi300x/mi300x-17.webp)
[Source: SemiAnalysis]
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-18.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-20.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-21.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-22.webp)


### 14. All Reduce/All to All/Reduce Scatter/All Gather Collectives 개요 ###
...
![Source: DeepSpeed](./20241222-mi300x/mi300x-23.webp)

![Source: Meta](./20241222-mi300x/mi300x-24.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-25.webp)

![Source: SemiAnalysis](./20241222-mi300x/mi300x-26.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-29.webp)
...
![Source: Nvidia](./20241222-mi300x/mi300x-30.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-31.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-32.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-35.webp)

![Source: SemiAnalysis](./20241222-mi300x/mi300x-36.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-37.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-38.webp)
...
![Source: PyTorch/AMD](./20241222-mi300x/mi300x-43.webp)
...
![Source: SemiAnalysis](./20241222-mi300x/mi300x-45.webp)
...
AMD는 Meta와 협력하여 ROCm에서 프로덕션 훈련 워크로드가 작동하도록 해야 한다. PyTorch 사용자들 사이에서 잘 알려진 사실은, Meta가 내부적으로 사용하지 않는 PyTorch 코드 경로에는 수많은 버그가 존재한다는 점이다. 현재 Meta는 MI300X 추론을 위해 HIP 커널을 수작업으로 작성하지만, 실제 훈련에는 MI300X를 사용하지 않는다. 만약 다음 Llama의 소형 버전을 AMD에서 훈련한다면, 이는 AMD 생태계에 큰 개선을 가져오고 마케팅 측면에서도 승리가 될 것이다. 이를 통해 AMD가 Meta와 함께 더 큰 모델/클러스터로 점진적으로 나아갈 수 있는 문을 열 수 있다. Meta가 실제 모델 훈련에 AMD GPU를 사용하는 것은 양사 모두에게 이익이 될 것이다. Meta 또한 NVIDIA의 대안이 될 수 있는 훈련 칩을 찾고 있기 때문이다.

현재 NVIDIA는 외부 PyTorch 지속적 개선(CI/CD) 및 개발을 위해 1,000개 이상의 GPU를 제공하며, 내부적으로는 이보다 훨씬 더 많은 GPU를 사용하고 있다. 그러나 AMD는 그렇지 않다. AMD는 AMD에 집중된 GPU 네오클라우드와 협력하여 각 세대당 약 10,000개의 GPU를 내부 개발 및 PyTorch용으로 확보해야 한다. 이는  NVIDIA의 다가오는 대규모 블랙웰 클러스터에 비해 여전히 1/8 수준이지만, 시작점으로는 충분하다. 이 GPU들은 내부 개발 및 PyTorch CICD에 전용될 수 있다.

**“리사, AMD의 데이터센터 GPU 사용자 경험을 개선하는 방법에 대해 논의할 회의에 열려 있어요!"**


### 21. H100/H200/MI300X 네트워킹 BoM 분석 및 TCO 대비 성능 ###


집합 연산 및 GEMM 처리량 벤치마킹 외에도, 클러스터에서 실제 워크로드를 실행하고 추가 벤치마크를 수행하기 위한 유용한 주제를 탐구하는 여러 실험을 진행했다. 이러한 실험은 벤치마킹 워밍업 및 반복 효과, VBoost 전력 이동, MLPerf Training GPT-3, BF16과 FP16 처리량 비교, GEMM 입력 분포별 처리량, FLOP당 전력 소모, 그리고 PyTorch PyPi 배포판과 NVIDIA NGC Stable PyTorch 이미지 간의 처리량 비교를 다룬다.

또한, 1k GPU 이더넷, 1k GPU 인피니밴드, 16k GPU 이더넷, 16k GPU 인피니밴드 클러스터에 대한 네트워킹 비용(Bill of Materials, BoM) 분석을 상세히 제공한다. 이와 함께 백엔드 네트워킹에서 51.2T Radix 스위치와 25.6T Radix 스위치 사용의 영향을 논의한다.

마지막으로, H100/H200/MI300X가 실질적인 훈련 페타플롭 기준으로 $/시간 대비 성능에서 어떻게 비교되는지 보여주는 TCO 대비 성능 분석을 제공한다. 이러한 내용은 SemiAnalysis 구독자들에게 제공되며 데이터센터 운영자, ML 과학자, 투자자들에게 매우 흥미로운 자료가 될 것이다.

|<span style="color:green">레퍼런스:</span>https://semianalysis.com/2024/12/22/mi300x-vs-h100-vs-h200-benchmark-part-1-training/|
